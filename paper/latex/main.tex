...
signal. Table~\ref{tab:baselines-newrecur} decomposes each learned model's
...
\begin{itemize}
  ...
  \item \textbf{Untuned baseline calibration.} The XGBoost baseline
    ...
    across all 470 labels with no per-label calibration or hyperparameter
    search; its poor point-prediction F1 despite the best ranking metrics of
    any model tested is therefore a lower bound on what a fully-tuned
    flat-feature baseline could achieve, not necessarily its ceiling.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Per-label-calibrated flat-feature baselines.} Re-running the
    XGBoost baseline (Section~5.2) with per-label \texttt{scale\_pos\_weight}
    or post-hoc probability calibration would test whether its
    point-prediction F1 gap relative to BEHRT is a tuning artifact or a
    genuine architectural limitation of non-sequential models.
\end{enumerate}
