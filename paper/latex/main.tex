...
\usepackage{graphicx}
\graphicspath{{figures/}{paper/latex/figures/}}
...
\begin{abstract}
Longitudinal prediction of which clinical codes will appear at a patient's
next visit is a central problem in clinical informatics, yet most electronic
health record (EHR) models trained on large, general-population cohorts do
not characterize \emph{what kind of task} they are actually solving. We adapt
BEHRT, a BERT-style transformer for EHR sequences, to a smaller,
higher-acuity cohort: 24{,}729 critically ill patients (179{,}713
training/evaluation samples) drawn from a MIMIC-IV-derived ICU population,
with diagnoses grouped into 463 Clinical Classifications Software Refined
(CCSR) categories. Our reference model (6 transformer layers, 12 attention
heads, 288-dimensional hidden space) is pre-trained with a masked language
model (MLM) objective and fine-tuned for multi-label next-visit prediction
with positive-class weighting for rare codes, reaching a sample-wise ROC-AUC
of 0.901 and Average Precision Score (APS) of 0.274 on held-out test data.

A single AUC/APS pair, however, hides more than it reveals for a task like
this. Eleven exploratory data analyses show that 70\% of correct predictions
are simply recurrences of a patient's already-known conditions rather than
new-disease detection; that performance ranges from F1 = 0.386 on
administrative codes to F1 = 0.046 on injury codes; and that longer diagnosis
histories keep improving performance even after truncation to the model's
64-token context window. Two independent interpretability techniques --
Integrated Gradients and attention rollout -- corroborate that over half
(53.3\%) of the model's false positives reflect genuine cross-category
clinical comorbidity rather than superficial code-similarity confusion.

Most importantly, we benchmark BEHRT against two non-learned baselines and
two flat-feature learned baselines (logistic regression and XGBoost) built
from simple per-patient history features. Bootstrap resampling confirms this
comparison is not sampling noise ($p<0.001$ for every pairwise ranking-metric
delta): both flat baselines match or exceed BEHRT on aggregate ranking
metrics, and we show this gap is not explained by undertuned decision
thresholds alone -- an exhaustive per-label threshold sweep recovers only
+0.008 absolute micro-F1 for XGBoost, far short of closing its point-prediction
gap to BEHRT. Instead, BEHRT's real, defensible advantage is narrower and more
specific: it is the best model at detecting genuinely new diagnoses (F1 =
0.061, AUC = 0.865 on this 30\% subset) and the only one that yields usable,
well-calibrated point predictions from a single decision threshold. We argue
this evidence supports a precise, falsifiable claim: BEHRT-style transformers
learn genuine temporal patterns of clinical code recurrence and comorbidity in
multimorbid ICU patients, and should be framed as care-coordination support
tools rather than general disease-onset predictors.
\end{abstract}
...
Calibration and long-tail multi-label learning bear directly on two of this
work's central findings. Guo et al.~\cite{guo2017calibration} showed that
modern deep networks are frequently miscalibrated even when their ranking
metrics (e.g.\ AUC) are strong, which is precisely the pattern we observe for
the XGBoost baseline (Section~5.2): the best ranking metrics of any model
tested, yet the worst point-prediction micro-F1, once a single operating
threshold is fixed. In our setting the deeper cause is a single
dataset-wide \texttt{scale\_pos\_weight} applied at training time across 470
labels of widely differing base rates, rather than temperature miscalibration
per se, but the diagnosis -- discrimination and calibration are separate
properties that can diverge sharply -- is the same one this literature
establishes. Separately, the long-tail and imbalanced multi-label
classification literature, including distribution-balanced loss reweighting
for multi-label settings~\cite{wu2020distributionbalanced} and logit
adjustment for long-tailed recognition~\cite{menon2021logitadjustment},
targets exactly the label-frequency skew (spanning more than three orders of
magnitude across our 463 active CCSR categories) that produces the structural
micro-F1 = 0 floor for ultra-rare labels identified in Section~5.6. Neither
technique is applied in this work; both are natural candidates for the
few-shot and transfer-learning remedies proposed as future work in Section~8.
...
To confirm these ranking-metric gaps -- and the micro-F1 gaps discussed
above -- are not an artifact of sampling noise in a single fixed test split,
we computed 500-resample bootstrap 95\% confidence intervals for APS,
ROC-AUC, \emph{and} micro-F1 for every model, together with paired
...
twelve paired deltas (APS, AUC, and F1, four baselines) are statistically
significant at $p<0.001$. In particular, BEHRT's micro-F1 deficit relative to
logistic regression ($-0.0585$, 95\% CI $[-0.0605,-0.0562]$) and its micro-F1
advantage over XGBoost ($+0.0914$, 95\% CI $[0.0891,0.0938]$) are both
statistically robust rather than sampling noise -- so the aggregate micro-F1
ranking (logistic regression $>$ BEHRT $>$ XGBoost) is a real effect that
must be explained, not explained away. This is the motivation for the
new-vs-recurring decomposition below, which locates \emph{where} BEHRT's
real advantage lies rather than disputing that the aggregate ranking exists.
...
\begin{tabular}{lcccc}
\toprule
Model & APS (95\% CI) & ROC-AUC (95\% CI) & Micro-F1 (95\% CI) & $p$ vs.\ BEHRT (APS, AUC, F1) \\
\midrule
XGBoost & 0.4894 (0.4863, 0.4924) & 0.9307 (0.9297, 0.9318) & 0.1467 (0.1455, 0.1479) & $<0.001$, $<0.001$, $<0.001$ \\
Logistic regression & 0.2915 (0.2890, 0.2939) & 0.9162 (0.9153, 0.9171) & 0.2966 (0.2946, 0.2986) & $<0.001$, $<0.001$, $<0.001$ \\
BEHRT (reference) & 0.2735 (0.2708, 0.2762) & 0.9006 (0.8997, 0.9016) & 0.2381 (0.2358, 0.2403) & -- \\
Popularity & 0.2544 (0.2521, 0.2569) & 0.8914 (0.8902, 0.8924) & 0.2716 (0.2691, 0.2740) & $<0.001$, $<0.001$, $<0.001$ \\
Repeat last visit & 0.2372 (0.2346, 0.2398) & 0.7184 (0.7165, 0.7202) & 0.4264 (0.4237, 0.4292) & $<0.001$, $<0.001$, $<0.001$ \\
\bottomrule
\end{tabular}%
}
\end{table}

Figure~\ref{fig:bootstrap-ci} provides the same bootstrap uncertainty
information as Table~\ref{tab:bootstrap} in compact visual form, making
the non-overlapping effect sizes around BEHRT easier to inspect at a glance.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{fig_bootstrap_ci.png}
\caption{Bootstrap 95\% confidence intervals for APS, ROC-AUC, and micro-F1
across all models (500 paired resamples).}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.94\textwidth]{fig_new_vs_recurring.png}
\caption{Recurring-vs-new decomposition for learned models.
Left: micro-F1. Right: ROC-AUC. BEHRT's relative strength is concentrated
in new-diagnosis detection.}
\label{fig:new-vs-recurring}
\end{figure}
...
a matched baseline run (\texttt{use\_pos\_weight=False}) across three random
seeds (42, 43, 44), each pair sharing the same seed, data split, label
vocabulary, and \emph{current} evaluation schema. Table~\ref{tab:posweight}
reports the per-seed comparison and the aggregate effect size.

\begin{table}[h]
\centering
\caption{Multi-seed effect of positive-class weighting (3 seeds, matched
data/schema).}
\label{tab:posweight}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcccccc}
\toprule
Seed & APS (base) & APS (weighted) & $\Delta$APS & AUC (base) & AUC (weighted) & $\Delta$AUC \\
\midrule
42 & 0.2543 & 0.2735 & +0.0193 & 0.8911 & 0.9006 & +0.0096 \\
43 & 0.2573 & 0.2718 & +0.0144 & 0.8910 & 0.9014 & +0.0105 \\
44 & 0.2544 & 0.2814 & +0.0269 & 0.8914 & 0.9034 & +0.0120 \\
\midrule
Mean $\pm$ SD & 0.2553 & 0.2756 & \textbf{+0.0202 $\pm$ 0.0063} & 0.8912 & 0.9018 & +0.0107 $\pm$ 0.0012 \\
\bottomrule
\end{tabular}%
}
\end{table}

The direction of the ranking-metric improvement is consistent across all
three seeds, confirming that the earlier single-seed +7.6\% relative APS
estimate (seed 42 alone) is not an artifact of seed variance: the mean
relative APS change across the three seeds is +7.9\%\ (individual seeds:
+7.6\%, +5.6\%, +10.6\%), and every seed shows a positive $\Delta$APS and
$\Delta$AUC.

However, the same three seeds reveal a trade-off that was not visible in
the original single-seed comparison. Positive-class weighting shifts the
F1-optimal decision threshold from 0.15 (baseline) to 0.60 (weighted), and
at these respective tuned thresholds, test-set micro-F1 is consistently
\emph{lower} for the weighted arm: baseline mean F1 = 0.2716 (identical to
4 decimal places across all three seeds) vs.\ weighted mean F1 = 0.2416
$\pm$ 0.0067 (per-seed $\Delta$F1: $-$0.0335, $-$0.0343, $-$0.0223). All
micro-F1 figures reported elsewhere in this paper (e.g.\
Table~\ref{tab:bootstrap}) use the positive-class-weighted reference
model, so its F1 should be read as the outcome of an explicit
ranking-vs.\ point-decision trade-off rather than evidence that weighting
uniformly improves every metric. This verified result supersedes an
earlier, unverified internal estimate of +4.6\% APS, which mixed runs from
the legacy evaluation schema.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{fig_posweight_tradeoff.png}
\caption{Per-seed effect of positive-class weighting (weighted minus
baseline) across APS, ROC-AUC, and tuned-threshold micro-F1. Ranking
metrics are consistently positive, while micro-F1 is consistently negative.}
\label{fig:posweight-tradeoff}
\end{figure}
...
Positive-class weighting provides a verified, multi-seed (n=3) +7.9\% mean
APS improvement for rare-code detection on top of this, at the cost of a
consistent reduction in tuned-threshold micro-F1.
...
\begin{itemize}
  ...
  \item \textbf{Non-disease target codes.} 14.3\% of targets are symptom,
    ...
    disease codes.
  \item \textbf{Positive-class weighting trades F1 for ranking quality.}
    Across 3 seeds (42, 43, 44) under the current evaluation schema,
    positive-class weighting consistently improves ranking metrics (mean
    $\Delta$APS~=~+0.0202~$\pm$~0.0063, mean $\Delta$AUC~=~+0.0107~$\pm$~0.0012)
    but consistently \emph{reduces} tuned-threshold micro-F1 (mean
    $\Delta$F1~=~$-$0.0300~$\pm$~0.0067). The reference model used throughout
    this paper is the positive-class-weighted variant, so its F1 figures
    should be interpreted alongside this trade-off, not as a strictly
...
\begin{enumerate}
  \item \textbf{Recovering F1 without sacrificing ranking quality} for
    positive-class weighting: the multi-seed comparison in Section~5.3
    shows a consistent APS/AUC gain but a consistent tuned-threshold F1
    loss; per-label (rather than dataset-wide) weighting, or a
    recurrence-aware threshold policy (see below), may recover the F1 lost
    to the current single global weighting scheme.
...
superiority. Positive-class weighting provides a verified multi-seed
(42/43/44) ranking gain (+7.9\% mean relative APS), with a corresponding
tuned-threshold micro-F1 trade-off that should be interpreted explicitly.
Two independent
...
\bibitem{wu2020distributionbalanced}
T.~Wu, Q.~Huang, Z.~Liu, Y.~Wang, and D.~Lin, ``Distribution-balanced loss
for multi-label classification in long-tailed datasets,'' in \emph{Proceedings
of the European Conference on Computer Vision (ECCV)}, 2020, pp.~162--178.

\bibitem{menon2021logitadjustment}
